...
def analyze_text_requirements(bedrock, user_text: str, system_text: Optional[str], source: str, source_file: str, debug: bool = False) -> List[Dict[str, Any]]:
	...
	instruction = (
		...
		"customer (customer name if found in row, otherwise empty), material (material/product ID or code - numeric part only), quantity, unit, delivery_date (date in YYYY-MM-DD format), urgency, notes, source, source_file, row_index. "
...
def analyze_image_requirements(
	...
	prompt = (
		...
		"Return ONLY JSON (no markdown), as a list of objects with keys: "
		"customer (customer name if found in row, otherwise empty), material, quantity, unit, delivery_date, urgency, notes, source, source_file, row_index. "
		"IMPORTANT: Do NOT extract generic phrases like 'Material Requirements', 'Customer Material Requirements', 'Requirements', or 'Material' as customer names. "
		"Only extract actual company/customer names (e.g., 'ABC Pvt Ltd', 'Cipla', 'ENCUBE ETHICALS PVT LTD'). "
		"If customer name is not found in the row data, leave customer field empty (it will be filled from email sender). "
