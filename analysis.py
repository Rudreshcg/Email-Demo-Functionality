...
def analyze_text_requirements(bedrock, user_text: str, system_text: Optional[str], source: str, source_file: str, debug: bool = False) -> List[Dict[str, Any]]:
	...
	instruction = (
		...
		"customer (customer name or email), material (material/product ID or code), quantity, unit, delivery_date (date), urgency, notes, source, source_file, row_index. "
		"For the material field, extract the product ID, material code, or SKU if available. "
...
def analyze_image_requirements(
	...
	prompt = (
		...
		"Return ONLY JSON (no markdown), as a list of objects with keys: "
		"customer, material, quantity, unit, delivery_date, urgency, notes, source, source_file, row_index. "
